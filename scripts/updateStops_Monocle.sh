...
madconf=config.ini
gymfile=updateStops.csv
errorfile=updateStops.csv.error
...
id=$(( $(query "select id from pokestops order by id desc limit 1;") + 1 ))
...
done < <(query "select url from pokestops where name='$name'")
 echo "Inserting stop id $id: $name2"
 if query "insert into pokestops (id, external_id, lat, lon, name, url, updated, quest_id, reward_id,deployer,lure_start,expires,quest_submitted_by, edited_by) values ($id,$id,$lat,$lon,'${name}','${url}',NULL,NULL,NULL,NULL,NULL, NULL, NULL, NULL);" 2>/dev/null;then
   echo "success."
 else erroroutput
 fi
 id=$(( 1 + $id ))
